...
*extra check: household income in quartiles instead of tertiles.
foreach var in i.HHIncT i.HHIncQ {
regress cc_wtstig i.Rage_cat Rsex `var'
}
...
*own perception of shape?fre complete-case
...
clear all
*cd "BSAS_project"
cd "C:\Users\sh5327\OneDrive - University of Bristol\SURVEYS AND DATA\BSAS 2015"
...
eststo base: mi estimate, post: regress zwtstig i.Rage_cat i.Rsex 
...
label variable rescale_bmi "Own BMI, per 5kg/m{superscript:2}"
label variable DisNew2 "Has a longterm illness/disability"
...
label variable zleftr "Left-right (higher values: rightwing)"
label variable zliba "Libertarian-authoritarian (higher values: authoritarian)"
label variable zwelfst "Welfare-stigmatizing attitudes"

*demographic:
coefplot base, /// 
keep (*Rage* *Rsex*) xline(0) ///
rename (1.Rage_cat =  "18-29 years" 2.Rage_cat = "30-39 years" 3.Rage_cat = "40-49 years" 4.Rage_cat = "50-59 years" 5.Rage_cat = "60-69 years" 6.Rage_cat = "70-79 years" 7.Rage_cat = "80+ years" 2.Rsex = "women") ///
coeflabels (,wrap(40) labsize(vsmall)) ///
legend(off) grid(none) scheme (s1mono) graphregion (color(white)) ///
/* title ("{bf}Fig 1: Demographic differences: weight-stigmatizing index*", pos(11) size(small)) /// */ xtitle("Standardized difference in index of weight-stigmatizing attitudes", size(small)) ///
xscale(titlegap(2)) xlabel(, labsize(small)) ///
note("All models include age and gender. Reference age group: 18-29. Reference gender: men. All indexes are standardized.", pos(6) size(vsmall)) 
graph save "results/graphs/Figure_1_demographic_differences_zwtstig.gph", replace
graph export results/graphs/Figure_1_demographic_differences_zwtstig.tif, replace width(1200)
...
1.HHIncT = "Household income: lowest tertile" 2.HHIncT = "Household income: middle tertile" 3.HHIncT = "Household income: highest tertile" ///
1.SRInc_REV = "Subjective income: low" 2.SRInc_REV = "Subjective income: middle" 3.SRInc_REV = "Subjective income: high" ///
1.RNSSECG_5grp = "NS-SEC: Higher managerial, administrative and professional occupations" 2.RNSSECG_5grp = "NS-SEC: Intermediate occupations" 3.RNSSECG_5grp = "NS-SEC: Small employers/own account" 4.RNSSECG_5grp ="NS-SEC: Lower supervisory/technical" 5.RNSSECG_5grp ="NS-SEC: Semi-routine/routine" 6.RNSSECG_5grp ="NS-SEC: not classifiable" ///
1.REconAct_v3 = "Employment status: employed" 2.REconAct_v3 ="Employment status: unemployed" 3.REconAct_v3="Employment status: not working due to sickness/disability" 4.REconAct_v3="Employment status: other") ///
coeflabels (,wrap(40) labsize(vsmall)) ///
legend(off) grid(none) scheme (s1mono) graphregion (color(white)) ///
/* title ("{bf}Fig 2: Socioeconomic differences: weight-stigmatizing index*", pos(11) size(small)) /// */ xtitle("Standardized difference in index of weight-stigmatizing attitudes", size(small)) ///
xscale(titlegap(2)) xlabel(, labsize(small)) ///
note("All models include age and gender. Reference group, education: has a degree." "Reference, household income tertile: lowest. Reference, subjective income: low." "Reference, NS-SEC: 1&2. Reference, employment status: employed/self-employed." "All indexes are standardized.", pos(6) size(vsmall)) 
graph save "results/graphs/Figure_2_socioeconomic_differences_zwtstig.gph", replace
graph export results/graphs/Figure_2_socioeconomic_differences_zwtstig.tif, replace width(1200)

*own bmi and health:
coefplot rescale_bmi DisNew2 happywtr viewwt_v3, /// 
keep (rescale_bmi *DisNew2* happywtr *viewwt_v3*) xline(0) ///
rename(rescale_bmi = "Own BMI, per 5kg/m{superscript:2}" ///
1.DisNew2 = "Has a longterm illness/disability" ///
happywtr = "Happiness with own weight" ///
0.viewwt_v3 =  "Self-perception: about right weight" 1.viewwt_v3 = "Sees self as underweight" 2.viewwt_v3 = "Sees self as a bit overweight" 3.viewwt_v3 = "Sees self as very overweight") ///
coeflabels (,wrap(40) labsize(vsmall)) ///
legend(off) grid(none) scheme (s1mono) graphregion (color(white)) ///
/* title ("{bf}Fig 3: Own weight and health: differences in weight-stigmatizing index*", pos(11) size(small)) /// */ xtitle("Standardized difference in index of weight-stigmatizing attitudes", size(small)) ///
xscale(titlegap(2)) xlabel(, labsize(small)) ///
note("All models include age and gender. Reference group for longterm illness/disability: none." "Reference group for perception of own weight: about the right weight. All indexes are standardized.", pos(6)  size(vsmall)) 
graph save "results/graphs/Figure_3_ownbmihealth_differences_zwtstig.gph", replace
graph export results/graphs/Figure_3_ownbmihealth_differences_zwtstig.tif, replace width(1200)
...
coeflabels (,wrap(40) labsize(vsmall)) ///
legend(off) grid(none) scheme (s1mono) graphregion (color(white)) ///
/*title ("{bf}Fig 4: Beliefs and attitudes: differences in weight-stigmatizing index*", pos(11) size(small)) /// */ xtitle("Standardized difference in index of weight-stigmatizing attitudes", size(small)) ///
xscale(titlegap(2)) xlabel(, labsize(small)) ///
note("All models include age and gender. All indexes are standardized.", pos(6) size(vsmall)) 
graph save "results/graphs/Figure_4_attitudinal_differences_zwtstig.gph", replace
graph export results/graphs/Figure_4_attitudinal_differences_zwtstig.tif, replace width(1200)
...
dem_viewwt_v3 ="+ perception of own weight" ///
...
dem_zleftr = "+ left-right values" ///
dem_zliba = "+ libertarian-authoritarian values" ///
dem_zwelfst = "+ welfare-stigmatizing attitudes" ///
dem_all = "+ full adjustment", regex) ///
coeflabels (,wrap(40) labsize(vsmall)) ///
legend(off) grid(none) scheme (s1mono) graphregion (color(white)) ///
/*title ("{bf}Fig 5a: Age differences in weight-stigmatizing index*", pos(11) size(small)) /// */ xtitle("Standardized difference in index of weight-stigmatizing attitudes", size(small)) ///
xscale(titlegap(2)) xlabel(, labsize(small)) ///
note ("Coefficients show differences by age group (reference: 18-29 years). All indexes are standardized.", pos(6) size(vsmall))
*save it
graph save "results/graphs/Figure_5a_age_pt1_mediation_zwtstig.gph", replace
graph export results/graphs/Figure_5a_age_pt1_mediation_zwtstig.tif, replace width(1200)
...
coeflabels (,wrap(40) labsize(vsmall)) ///
legend(off) grid(none) scheme (s1mono) graphregion (color(white)) ///
/* title ("{bf}Fig 5b: Age differences in weight-stigmatizing index*", pos(11) size(small)) /// */ xtitle("Standardized difference in index of weight-stigmatizing attitudes", size(small)) ///
xscale(titlegap(2)) xlabel(, labsize(small)) ///
note ("Coefficients show differences by age group (reference: 18-29). All indexes are standardized.", pos(6) size(vsmall))
*save it 
graph save "results/graphs/Figure_5b_age_pt2_mediation_zwtstig.gph", replace
graph export results/graphs/Figure_5b_age_pt2_mediation_zwtstig.tif, replace width(1200)
...
coeflabels (,wrap(40) labsize(vsmall)) ///
legend(off) grid(none) scheme (s1mono) graphregion (color(white)) ///
/*title ("{bf}Fig 6: Gender differences in weight-stigmatizing index*", pos(11) size(small)) /// */ xtitle("Standardized difference in index of weight-stigmatizing attitudes", size(small)) ///
xscale(titlegap(2)) xlabel(, labsize(small)) ///
xscale(range(-0.36 0.02) titlegap(2)) xlabel(-0.35 -0.30 -0.25 -0.20 -0.15 -0.10 -0.05 0, labsize(small)) ///
note ("Coefficients show differences by gender (women vs men). All indexes are standardized.", pos(6) size(vsmall))
*save it
graph save "results/graphs/Figure_6_gender_mediation_zwtstig.gph", replace
graph export results/graphs/Figure_6_gender_mediation_zwtstig.tif, replace width(1200)
...
rename(2.HEdQual_3grp = "qualifications below degree" 3.HEdQual_3grp = "no qualifications") ///
eqrename (educ_base = "Adjusted for: age, gender" ///
educ_rescale_bmi = "+ own BMI" ///
educ_viewwt_v3 ="+ perception of own weight" ///
...
educ_zleftr = "+ left-right values" ///
educ_zliba = "+ libertarian-authoritarian values" ///
educ_zwelfst = "+ welfare-stigmatizing attitudes" ///
educ_all = "+ full adjustment", regex) ///
coeflabels (,wrap(40) labsize(vsmall)) ///
legend(off) grid(none) scheme (s1mono) graphregion (color(white)) ///
/*title ("{bf}Fig 7: Educational differences in weight-stigmatizing index*", pos(11) size(small)) /// */ xtitle("Standardized difference in index of weight-stigmatizing attitudes", size(small)) ///
xscale(titlegap(2)) xlabel(, labsize(small)) ///
note ("Reference category: has a university degree. All indexes are standardized.", pos(6) size(vsmall))
*save it
graph save "results/graphs/Figure_7_educ_mediation_zwtstig.gph", replace
graph export results/graphs/Figure_7_educ_mediation_zwtstig.tif, replace width(1200)
...
rename(1.HHIncT = "lowest tertile" 2.HHIncT ="middle tertile" 3.HHIncT ="highest tertile") ///
eqrename (objinc_base = "Adjusted for: age, gender" ///
objinc_rescale_bmi = "+ own BMI" ///
objinc_viewwt_v3 ="+ perception of own weight" ///
...
objinc_zleftr = "+ left-right values" ///
objinc_zliba = "+ libertarian-authoritarian values" ///
objinc_zwelfst = "+ welfare-stigmatizing attitudes" ///
objinc_all = "+ full adjustment", regex) ///
coeflabels (,wrap(40) labsize(vsmall)) ///
legend(off) grid(none) scheme (s1mono) graphregion (color(white)) ///
/* title ("{bf}Fig 8: Weight-stigmatizing attitudes by household income tertile*", pos(11) size(small)) /// */ xtitle("Standardized difference in index of weight-stigmatizing attitudes", size(small)) ///
xscale(titlegap(2)) xlabel(, labsize(small)) ///
note ("Reference category: lowest tertile. All indexes are standardized.", pos(6) size(vsmall))
*save it
graph save "results/graphs/Figure_8_objinc_mediation_zwtstig.gph", replace
graph export results/graphs/Figure_8_objinc_mediation_zwtstig.tif, replace width(1200)
...
keep(*HHIncT*) ///
replace title(Household income tertile associations with index: attenuation with adjustment) ///
note("All models include age and gender") 
*will need to transpose....
...
subjinc_viewwt_v3 ="+ perception of own weight" ///
...
subjinc_zleftr = "+ left-right values" ///
subjinc_zliba = "+ libertarian-authoritarian values" ///
subjinc_zwelfst = "+ welfare-stigmatizing attitudes" ///
subjinc_all = "+ full adjustment", regex) ///
coeflabels (,wrap(40) labsize(vsmall)) ///
legend(off) grid(none) scheme (s1mono) graphregion (color(white)) ///
/* title ("{bf}Fig 9: Weight-stigmatizing attitudes by subjective income*", pos(11) size(small)) /// */ xtitle("Standardized difference in index of weight-stigmatizing attitudes", size(small)) ///
xscale(titlegap(2)) xlabel(, labsize(small)) ///
note ("Reference category: 'low' subjective income. All indexes are standardized.", pos(6) size(vsmall))
*save it
graph save "results/graphs/Figure_9_subjinc_mediation_zwtstig.gph", replace
graph export results/graphs/Figure_9_subjinc_mediation_zwtstig.tif, replace width(1200)
...
keep(*SRInc_REV*) ///
replace title(Subjective income associations with index: attenuation with adjustment) ///
note("All models include age and gender") 
*will need to transpose....
...
note("Odds ratios, from ordered logistic regression. All models include age and gender" pos(6) size(vsmall)) 
...
note ("*All models include age and gender. Outcomes and indexes are standardized", pos(6) size(vsmall))
...
"NSSEC 2" "NSSEC 3" "NSSEC 4" "NSSEC 5" "NSSEC unclassifiable" ///
"Employment - unemployed" "Employment - sick/disability" "Employment - other" ///
"Own BMI (per kg/m2)" ///
"Sees self as underweight" "Sees self as a bit overweight" "Sees self as very overweight" ///
"Happiness with own weight" ///
"Has long-term illness/disability" ///
"Importance of inheritance" "Importance structural factors" "Health consequences obesity" "Left-right values" "Libertarian-authoritarian values" "Welfare-stigmatizing attitudes"
*check
matrix list itemcoeffs

*heatplot:
heatplot itemcoeffs, values(format(%9.2f)) ylabel(, labsize(vsmall)) xlabel(, labsize(small)) xscale(alt) colors(plasma) ///
aspectratio(1) legend(on subtitle("regression" "coefficient", size(vsmall)) region(lstyle(none))) graphregion (color(white)) graphregion(margin(l+10)) ///
title ("{bf}Figure S1: Standardized associations* with the index and its individual items", size(small)) ///
note("*All indexes, and all individual weight stigma items, are standardized." "All models include age and gender. Reference groups are age 18-29, men," "white ethnicity, university degree, low objective income, low subjective income," "NSSEC 1, employed, sees own weight as about right, no long-term illness/disability.", size(vsmall)) 

graph save "results/graphs/Fig_S1_heatplot_index_items.gph", replace
graph export results/graphs/Fig_S1_heatplot_index_items.tif, replace width(1200)
